...
window.PL_KB = [
  ...
  { topic: "NexusPay project",
    text: "NexusPay is Pierce's personal R&D project: a security-first payment-orchestration platform layered on HyperSwitch. It is a Spring Modulith (~16 modules) built in Java 21 and Spring Boot 3.2 with hexagonal architecture. Its headline strengths are zero-trust multi-tenancy (tenant is always derived from the authenticated principal — never a client header — with per-API-key scopes, and a whole class of cross-tenant/IDOR attacks driven to zero by red-team tests) and Stripe-test-mode-class developer experience (an in-process mock gateway that sk_test_ keys can never escape, forced decline/3DS/processing/fraud outcomes, test clocks, sandbox reset, and idempotency-key inspection). It also features a double-entry ledger with serializable transactions, a transactional outbox streamed to Kafka via Debezium change-data-capture, HMAC-signed and retried webhooks with a dead-letter queue, a fraud and sanctions engine, a dispute state machine, subscription billing with dunning, Temporal workflows, HashiCorp Vault for PCI-safe card handling (raw card numbers are never stored), and full Prometheus and Grafana observability. It ships three published TypeScript SDKs on npm (@nexus-pay/node, /js and /react at 0.1.2) plus a nexuspay CLI, and has a live marketing site and interactive UI at https://pierce-lonergan.github.io/NexusPay/. Honestly positioned, it is a payment-orchestration and reference layer that sits on top of HyperSwitch rather than a payment processor — it moves no real money itself. It is the most substantial of his personal projects, and it is a personal project, not JPMorganChase work." },

  { topic: "NexusMatcher project",
    text: "NexusMatcher is Pierce's personal R&D project: a semantic schema-matching system using multi-stage retrieval, late-interaction reranking, and learned type projections. It reranks 100 candidates in 3.17 milliseconds warm against 274 milliseconds cold, a 94 times speedup from precomputed token embeddings. Precision at 1 is 1.0 on the in-domain benchmark and 0.29 on a harder 17-field set, where Precision at 5 holds at 0.76. Pierce reports both numbers rather than only the flattering one. It has 433 tests, and it is a personal project, not JPMorganChase work." },
